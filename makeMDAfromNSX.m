function [] = makeMDAfromNSX(NSXfile,MDAfile)
% MAKEMDAFROMNSX makes an .mda file from the blackrock .nsx format
%   The inputs are references to a blackrock file (probably an .ns5 file)
%   and an .mda file for use with mountainlab/mountainsort

% version: [EHS20181105]

% loading the blackrock data.
NSX = openNSx(NSXfile,'read','report');

% TODO:: load geometry file to determine if the numbers of channels match
% for now, let's just assume that we're dealing with a utah array, as this
% is for spike sorting data.
if iscell(NSX.Data)
    if size(NSX.Data{2},1)>96
        data = NSX.Data{2}(1:96,:);
    end
else
    if size(NSX.Data{2},1)>96
        data = NSX.Data(1:96,:);
...
% saving data in MDA format.
dtype = 'int16';
if iscell(NSX.Data)
    writemda(NSX.Data{2},MDAfile,dtype);
else
    writemda(NSX.Data,MDAfile,dtype);
end
fprintf('\n*** .mda file ********************** \n%s\n\n',MDAfile)
end
...
NSx.MetaTags.openNSxver = '5.2.3.0';
